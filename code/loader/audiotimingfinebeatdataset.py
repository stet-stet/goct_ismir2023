...
from .beatencoder import NBeatEncoder, TwoTwoEncoderTimingOnly

def round_to_nearest_48th(num):
    return round(num*48)/48

class AudioTimingFineBeatDataset():
    def __init__(self, split_json_path="train.json", split_hdf5_path="train.h5", encoder="twotwo", 
        ...
        self.beat_encoder = TwoTwoEncoderTimingOnly()
        self.pad = 177
...
def stress_test(split="train", **params): 
    ...
    dset = AudioTimingFineBeatDataset(split_json_path=split_json_path, split_hdf5_path=split_hdf5_path, **params)
    print(len(dset))
...
if __name__=="__main__":
    ...
    #stress_test(center=True, token_length=100, truncate=47)
    size_test(center=True, token_length=100, truncate=47)

...
from .beatencoder import TwoTwoEncoderFrac

def round_to_nearest_48th(num):
    return round(num*48)/48

class AudioRandomShiftDataset():
    def __init__(self, split_json_path="train.json", split_hdf5_path="train.h5", encoder="twotwo", 
                 ...
                 logspec=False, center=False, truncate=None) -> None :
        ...
        self.beat_encoder = TwoTwoEncoderFrac()
        self.pad = 177
    ...
    def __getitem__(self,idx,benchmark=False) -> dict:
        ...
        random_nudge = random.randint(0,48)
        beat += random_nudge / 48
        ...
        if not dropmode:
            notes = list(filter(lambda x:(round_to_nearest_48th(x[1]) >= round_to_nearest_48th(beat) 
                                          and round_to_nearest_48th(x[1]) < round_to_nearest_48th(nextbeat)), notes))
        else:
            notes = list(filter(lambda x:(round_to_nearest_48th(x[1]) >= round_to_nearest_48th(beat+2) 
                                          and round_to_nearest_48th(x[1]) < round_to_nearest_48th(nextbeat)), notes))
...
if __name__=="__main__":
    # for split in ['valid']:
    #    stress_test(split
    stress_test(center=True, token_length=200, truncate=94)

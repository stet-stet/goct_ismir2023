...
class PlacementCNN(nn.Module):
    ...
    def dense(self, x_conv_diff: torch.Tensor):
        ...
        # Dense 0
        x_conv_diff = x_conv_diff.reshape( (-1, 1125) )
    ...
    def forward(
        ...
        for i in range(0, num_timesteps, dense_chunk_size):
            ...
            # print(f"x_chunk : {x_chunk.shape}")
            x_chunk = x_chunk.reshape(bs, -1, 1120)
            # print(f"x_chunk : {x_chunk.shape}")
            ...
            d = difficulties[:]
            # print(f"diff:{d.shape}")
            doh = self.conditions(d.unsqueeze(1)).float().unsqueeze(1)
            doh = doh.repeat((1,x_chunk.shape[1],1))
            # print(f"doh:{doh.shape}")
            # print(f"x_chunk:{x_chunk.shape}")
            x_chunk_diff = torch.cat([x_chunk, doh], dim=2)
            # print(f"x_chunk_diff : {x_chunk_diff.shape}")
